...
def main():
    ...
    if args.data_cluster == "adx":
        ...
        run_s3_process_snapshot(args.branch, "adx_data_regular.json adx_data_fund.json", "adx", ".tar.gz", 1)
        ...
        run_s3_process_snapshot(args.branch, "asx_descriptions.json", "asx", ".tar.gz", 1)
    elif args.data_cluster == "biva":
        biva_handler()
        delivery(["biva_data.csv"], args.branch)
        run_s3_process_snapshot(args.branch, "biva_data.json", "biva", ".tar.gz", 1)
        ...
        run_s3_process_snapshot(args.branch, "canadian_descriptions.json", "cse", ".tar.gz",1)
        ...
        run_s3_process_snapshot(args.branch, "factset_finra_isins.json", "finra", ".tar.gz", 1)
        ...
        run_s3_process_snapshot(args.branch, "LSX.csv", "lsx", ".tar.gz", 1)
    elif args.data_cluster == "LS":
        lang_and_shwarz_handler(args.data_cluster)
        delivery(["LS.csv"], args.branch)
        run_s3_process_snapshot(args.branch, "LSX.cs", "ls", ".tar.gz", 1)
        ...
        run_s3_process_snapshot(args.branch, nse_raw_listing, "nse", ".tar.gz", 1)
    elif args.data_cluster == "nasdaq_gids":
        nasdaq_gids_handler()
        delivery(["nasdaq_gids_symbols.csv"], args.branch)
        run_s3_process_snapshot(args.branch, "nasdaq_gids_symbols.csv", "gids", ".tar.gz", 1)
        ...
        run_s3_process_snapshot(args.branch, nasdaqtrader_descriptions, "nasdaq", ".tar.gz", 1)
    elif args.data_cluster == "nyse":
        nyse_handler()
        delivery(["nyse_data.csv", "amex_data.csv"], args.branch)
        run_s3_process_snapshot(args.branch, "nyse_data.csv", "nyse", ".tar.gz", 1)
        run_s3_process_snapshot(args.branch, "amex_data.csv", "amex", ".tar.gz", 1)
    elif args.data_cluster == "saudi":
        saudi_handler()
        delivery(["saudi_main_market.json", "saudi_nomu_parallel_market.json"], args.branch)
        run_s3_process_snapshot(args.branch, "saudi_main_market.json saudi_nomu_parallel_market.json", "amex", ".tar.gz", 1)
        ...
        run_s3_process_snapshot(args.branch, "taipei_descriptions.json taipei_local_descriptions.json", "sse", ".tar.gz", 1)
    else:
