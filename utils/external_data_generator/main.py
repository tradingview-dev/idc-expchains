...
def main(args, logger):
    ...
    return generate_and_upload(data_cluster, cluster_name, args.branch, not args.skip_check)
    ...
    for handler in data_cluster['handlers']:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar.gz", delete=False) as tmp_file:
            ...
            try:
                ...
                git_commit(files, git_branch, check_diff=False)
...
def generate_and_upload(data_cluster, cluster_name, git_branch, check_diff):
    files_to_commit = []
    for handler in data_cluster['handlers']:
        try:
    ...
    try:
        git_commit(files_to_commit, git_branch, check_diff)
    except Exception as e:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-check", action=argparse.BooleanOptionalAction, help="Skip check size before upload")
