...
from adx import ADXHandler
from asx import ASXHandler
from canada import canada_handler
from finra import finra_handler
from lang_and_shwarz import LangAndSchwarzHandler
from nasdaq_gids import NASDAQGIDSHandler
from nasdaqtrader import NASDAQTraderHandler
from saudi import SAUDIHandler
from shanghai import ShanghaiHandler
from taipei import TaipeiHandler
from tokyo import TokyoHandler
from twse import TwseHandler
...
def main(args, logger):
    ...
    data_clusters = {
        "adx": {"handler": ADXHandler().handle, "output": ["adx_data_regular.json", "adx_data_fund.json"]},
        "asx": {"handler": ASXHandler().handle, "output": ["asx_descriptions.csv"]},
        "biva": {"handler": biva_handler, "output": ["biva_data.csv"]},
        "canada": {"handler": canada_handler, "output": ["canadian_descriptions.json"]},
        "finra": {"handler": finra_handler, "output": ["factset_finra_isins.csv"]},
        "LSX": {"handler": LangAndSchwarzHandler().handle, "output": ["LSX.csv"], "path": "lsx"},
        "LS": {"handler": LangAndSchwarzHandler().handle, "output": ["LS.csv"], "path": "ls"},
        "nasdaq_gids": {"handler": NASDAQGIDSHandler().handle, "output": ["nasdaq_gids_symbols.csv"]},
        "nasdaqtrader": {"handler": NASDAQTraderHandler().handle, "output": ["nasdaqtrader_descriptions.txt"]},
        # "nyse": {"handler": NYSEHandler().handle, "output": ["nyse_data.csv", "amex_data.csv"]},
        "saudi": {"handler": SAUDIHandler().handle, "output": ["saudi_main_market.json", "saudi_nomu_parallel_market.json"]},
        "shanghai": {"handler": ShanghaiHandler().handle, "output": ["sse_descriptions.csv"]},
        "taipei": {"handler": TaipeiHandler().handle, "output": ["taipei_descriptions.json", "taipei_local_descriptions.json"]},
        "tokyo": {"handler": TokyoHandler().handle, "output": ["tokyo_local_descriptions.csv"]},
        "twse": {"handler": TwseHandler().handle, "output": ["twse_descriptions.csv"]},
        ...
        "korea": {"handler": TwseHandler().handle, "output": ["korea_local_descriptions.csv", "krx_derivatives_local_descriptions.csv"]}
    }
