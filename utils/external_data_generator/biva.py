...
class BivaDataGenerator(DataGenerator):
    ...
    async def __fetch_isin(self, session, url, isin_queue) -> None:
        ...
        async with session.get(url) as response:
            ...
            self._logger.info("Parsing ISIN data... ", False)
            try:
    ...
    def __write_result(self, symbols: list, isins: list) -> None:
        """
    ...
    def generate(self):
        try:
            ...
            self.__write_result(symbols, isins)
            return 0
...
if __name__ == "__main__":
    exit(BivaDataGenerator().generate())
