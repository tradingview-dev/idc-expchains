...
class TokyoHandler(Handler):

    BASE_URL = 'http://ca.image.jp/matsui/'
    ...
    def handle(self, data_cluster = None):
        ...
        for config in self.PAGE_TYPES.values():
            for page_number in range(1, config['pages'] + 1):
                ...
                try:
                    ...
                    self.parse_table_rows(results, resp.text, config.get('financial', False))
                except (RequestException, KeyError, TypeError) as e:
                    self._logger.error(e)
                    return 1

        filtered = self.filter_latest_entries(results)
        try:
            self.output_csv(filtered, "tokyo_local_descriptions.csv")
    ...
    @staticmethod
    def parse_table_rows(results, html, financial=False):
    ...
    @staticmethod
    def filter_latest_entries(results):
    ...
    @staticmethod
    def output_csv(filtered_results, filename):
